...
class DSLParser:
    ...
    def _min(*args):
        ...
        elif len(args) == 1 and hasattr(args[0], 'min'):
            return args[0].min()
        else:
    ...
    def _max(*args):
        ...
        elif len(args) == 1 and hasattr(args[0], 'max'):
            return args[0].max()
        else:
    ...
    @classmethod
    def _parse_node(cls, node: ast.AST, context: Dict[str, Any], df: Optional[pd.DataFrame] = None) -> Any:
        if isinstance(node, ast.Expression):
            return cls._parse_node(node.body, context, df)
            ...
            base = cls._parse_node(node.value, context, df)
            attr = node.attr
            ...
            if op_type in cls._BINARY_OPS:
                left = cls._parse_node(node.left, context, df)
                right = cls._parse_node(node.right, context, df)
            ...
            if op_type in cls._UNARY_OPS:
                operand = cls._parse_node(node.operand, context, df)
            ...
            if op_type == ast.And:
                ...
                for value in node.values:
                    val = cls._parse_node(value, context, df)
            ...
            for op, comp in zip(node.ops, node.comparators):
                right = cls._parse_node(comp, context, df)
                op_type = type(op)
                if op_type in cls._BINARY_OPS:
            ...
            if isinstance(node.func, ast.Name):
                ...
                if func_name in cls._FUNCTIONS:
                    args = [cls._parse_node(arg, context, df) for arg in node.args]
                    kwargs = {kw.arg: cls._parse_node(kw.value, context, df) for kw in node.keywords}
                    return cls._FUNCTIONS[func_name](*args, **kwargs)
                raise DSLError(f"不支持的函数: {func_name}")
            else:
                func_obj = cls._parse_node(node.func, context, df)
            ...
            if isinstance(node.slice, ast.Constant) and isinstance(node.slice.value, str):
                ...
                slice_ = cls._parse_node(node.slice, context, df)
            ...
            lower = cls._parse_node(node.lower, context, df) if node.lower else None
            upper = cls._parse_node(node.upper, context, df) if node.upper else None
            step = cls._parse_node(node.step, context, df) if node.step else None
            return slice(lower, upper, step)

        elif isinstance(node, ast.List):
            return [cls._parse_node(el, context, df) for el in node.elts]

        elif isinstance(node, ast.Tuple):
            return tuple(cls._parse_node(el, context, df) for el in node.elts)

        elif isinstance(node, ast.Dict):
            return {
                cls._parse_node(key, context, df): cls._parse_node(value, context, df)
    ...
    def evaluate(
        ...
        try:
            ...
            return cls._parse_node(tree, context, df)
...
class CohortDSLEngine:
    ...
    def apply_filter(self, df: pd.DataFrame, context: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        if not self.config.filter_expression:
            return df
        mask = DSLParser.evaluate(self.config.filter_expression, context, df)
    ...
    def compute_cohort_time(
        ...
        if self.config.cohort_expression:
            def _compute_cohort(group):
                ...
                if context:
                    ctx.update(context)
                return DSLParser.evaluate(self.config.cohort_expression, ctx, group)
    ...
    def compute_segment(
        ...
        result = DSLParser.evaluate(self.config.segment_expression, context, df)
